...
function Dashboard({ user, onLogout }) {
  ...
  async function addCard(cardId) {
    ...
    if (!response.ok) {
      ...
      setPremiumMessage(error.error || "Unable to add card");
      throw new Error(error.error || "Unable to add card");
    }
  ...
  async function activatePremiumDemo() {
    const response = await fetch(`${API_URL}/billing/demo-activate`, { method: "POST", headers: { Authorization: "Bearer " + token } });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Unable to activate Premium");
    setPremium(true);
    setPremiumMessage("Premium demo attivato: nessun pagamento reale effettuato.");
  }
  ...
  return <div className={dark ? "app" : "app light"}>
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><button className={active === "Premium" ? "nav-item active" : "nav-item"} onClick={() => setActive("Premium")}><Sparkles size={18}/>Premium</button><button className="nav-item"><Settings size={18}/>Settings</button><button className="nav-item"><CircleHelp size={18}/>Help center</button><div className="upgrade"><Sparkles size={18}/><b>{premium ? "Premium active" : "Unlock more insights"}</b><small>{premium ? "Unlimited collection enabled." : "€6/month · demo available."}</small><button onClick={() => setActive("Premium")}>Explore Premium</button></div></div>
    </aside>
    <main>
      ...
      <div className="content">
        ...
        {active === "Overview" && <><section className="stats"><Stat icon={Wallet} label="Portfolio value" value={`$${Number(summary.totalValue).toFixed(2)}`} change="+0.0%"/><Stat icon={TrendingUp} label="30d performance" value="+$0.00" change="No history yet"/><Stat icon={Layers3} label="Cards owned" value={summary.cardCount} change="Live portfolio"/><Stat icon={Trophy} label="Collection score" value="—" change="Add more cards" /></section>
          ...
          <section className="grid-two lower"><div className="panel"><div className="panel-head"><div><h2>Recently added</h2><p className="muted">Latest cards in your collection</p></div><button className="link" onClick={() => setActive("My collection")}>View collection</button></div><div className="card-grid">{portfolio.slice(0, 3).map(c => <CardTile key={c.id} card={c} onRemove={removeCard}/>)}</div>{portfolio.length === 0 && <p className="muted">Your collection is empty. Add your first card.</p>}</div><div className="panel insights"><div className="panel-head"><div><h2>Collection insights</h2><p className="muted">Personalized tips for you</p></div><Sparkles size={18} color="#a49aff"/></div><div className="insight"><div className="insight-icon purple-bg">↗</div><div><b>Build your first collection</b><p className="muted">Add cards to unlock portfolio trends.</p></div></div></div></section>
            ...
            {active === "My collection" && <section className="panel catalog-panel"><div className="panel-head"><div><h2>Your cards</h2><p className="muted">Stored in your PostgreSQL portfolio</p></div><button className="primary" onClick={() => setShowAdd(true)}><Plus size={15}/> Add cards</button></div>{premiumMessage && <div className="premium-message">{premiumMessage}</div>}<div className="catalog-grid">{portfolio.map(c => <CardTile key={c.id} card={c} onRemove={removeCard}/>)}</div>{portfolio.length === 0 && <div className="empty">No cards in your collection yet.</div>}</section>}
            {active === "Premium" && <section className="premium-page"><div className="premium-hero"><p className="eyebrow">LUMYN PRO</p><h1>Unlimited collecting.</h1><p className="muted">Unlock every collection feature with Premium.</p><strong>€6 <small>/ month</small></strong><button className="primary" onClick={activatePremiumDemo}>{premium ? "Premium active" : "Activate demo Premium"}</button><small className="muted">Demo mode: no real payment is processed.</small></div><div className="premium-features"><div><b>Unlimited portfolio</b><span>Save more than the 25-card free limit.</span></div><div><b>All tools included</b><span>Catalog, scanner, pricing and trade analysis.</span></div><div><b>Monthly subscription ready</b><span>Stripe recurring billing can be connected later.</span></div></div></section>}
